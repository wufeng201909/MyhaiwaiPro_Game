...
import com.myludo.MainActivity;
...
public class NotifyService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(@NonNull RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData().size() > 0) {
            ...
            MLog.a(TAG, "userId is " + userId);
            if (!userId.equals("")) {
                Map<String, String> data = remoteMessage.getData();
                String msgType = data.get("msg_type");
                String body = data.get("body");
                MLog.a(TAG, "msgType is " + msgType + " body is " + body);
        ...
        //键：link  值：  htttp://www.baidu.com
        MLog.a(TAG, "notification data is " + remoteMessage.getData());
        MLog.a(TAG, "notification link is " + remoteMessage.getData().get("link"));

        if (remoteMessage.getNotification() != null) {
            String notifyTitle = remoteMessage.getNotification().getTitle();
            String notifyBody = remoteMessage.getNotification().getBody();

            MLog.a(TAG, "notification title is " + notifyTitle);
            MLog.a(TAG, "notification body is " + notifyBody);


            MLog.a(TAG, "notification -----tag= " + remoteMessage.getNotification().getTag()
            +";link="+ remoteMessage.getNotification().getLink()+";Ticker="+ remoteMessage.getNotification().getTicker()
            +";BodyLocalizationKey="+ remoteMessage.getNotification().getBodyLocalizationKey()
            +";ImageUrl="+ remoteMessage.getNotification().getImageUrl()+";ChannelId="+ remoteMessage.getNotification().getChannelId()
            );
    ...
    public void onNewToken(@NonNull String refreshToken) {
        ...
        MLog.a(TAG, "refreshed token: " + refreshToken);
        // 1. 持久化生成的token，
        // 2. 发送事件通知RN层，分为两种情况：
